...
# ============================================================
# [1] Dynasty Waterwheel Evolution Analysis - Test Functions
# ============================================================
...
def t1_3():
    from evolution_analyzer import DynastyEvolutionAnalyzer
    a = DynastyEvolutionAnalyzer()
...
# ============================================================
# [2] Cross-Era Efficiency Comparison - Test Functions
# ============================================================

def t2_1():
    from era_comparator import CrossEraComparison
...
# ============================================================
# [3] Multi-Wheel Scheduling Optimization - Test Functions
# ============================================================

def _make_wheel(wid, lat=0, lon=0):
    from fleet_scheduler import WaterWheelUnit
...
def _make_zone(zid, area=2000, req=50, priority=3):
    from fleet_scheduler import IrrigationZone
...
def t3_1():
    from fleet_scheduler import MultiWheelScheduler
...
# ============================================================
# [4] Virtual Treading Experience - Test Functions
# ============================================================
...
def t4_4():
    from vr_waterwheel import TreadingPhysics
...
def t4_15():
    from vr_waterwheel import TreadingLeaderboard
...
# ============================================================
# [5] Refactored Modules - Unit Tests
# ============================================================

def t5_1():
    from evolution_analyzer import DynastyEvolutionAnalyzer, DynastyType
    a = DynastyEvolutionAnalyzer()
    r = a.get_dynasty_params(DynastyType.HAN)
    assert '考古数据源' in r
    assert '综合置信度' in r
    assert r['综合置信度'] > 0

def t5_2():
    from evolution_analyzer import DynastyType, DynastyParameters, ArchaeologicalSource
    src = ArchaeologicalSource("Test Ref", 2000, "test", 0.8, "test note")
    assert src.confidence == 0.8
    assert src.reference == "Test Ref"

def t5_3():
    from dataclasses import asdict
    from era_comparator import CrossEraComparison, CentrifugalPumpModel
    c = CrossEraComparison()
    r = c.compare_efficiency(12.0, 50.0)
    r_dict = asdict(r)
    assert 'pump_operating_validation' in r_dict
    assert r_dict['pump_operating_validation']['is_within_range'] is True

def t5_4():
    from era_comparator import OperatingRange, OperatingPointValidation, CentrifugalPumpModel
    m = CentrifugalPumpModel()
    rng = m.get_operating_range()
    assert rng.min_flow_m3h < rng.max_flow_m3h
    assert rng.best_efficiency_flow_m3h > 0

def t5_5():
    from fleet_scheduler import MultiWheelScheduler, CommunicationDelaySimulator
    s = MultiWheelScheduler()
    sim = CommunicationDelaySimulator()
    assert sim is not None
    assert sim.config is not None

def t5_6():
    from fleet_scheduler import CommunicationDelayConfig, CommunicationDelayResult
    config = CommunicationDelayConfig()
    assert config.base_delay_s > 0
    assert config.message_loss_rate >= 0

def t5_7():
    from fleet_scheduler import CommunicationDelaySimulator
    sim = CommunicationDelaySimulator()
    dist = sim.estimate_distance_km((34.0, 108.0), (34.1, 108.1))
    assert dist > 10.0
    assert dist < 20.0

def t5_8():
    from vr_waterwheel import TreadingExperienceManager, TreadingPhysics
    m = TreadingExperienceManager()
    p = TreadingPhysics()
    state = p.get_instantaneous_state(100.0, 60.0, 0.0, 0.0, 0.1)
    assert 'force_feedback' in state
    assert state['force_feedback']['pedal_resistance_n'] > 0

def t5_9():
    from vr_waterwheel import TreadingSession, TreadingLeaderboard
    lb = TreadingLeaderboard()
    assert lb is not None
    assert lb.get_top_n(5, 'water_lifted_liters') == []


# ============================================================
# [6] Mechanics Worker Process - Unit Tests
# ============================================================
...
def t6_2():
    from mechanics_worker import MechanicsWorker
    with MechanicsWorker() as w:
        assert w.is_alive() is True
        assert w._process.pid > 0
    assert w.is_alive() is False

def t6_3():
    from mechanics import WaterWheelSimulator
    from mechanics_worker import MechanicsWorker

    sim = WaterWheelSimulator()
    geom_dict = {
        'upper_wheel_diameter': 1.2, 'lower_wheel_diameter': 1.2,
        'center_distance': 4.0, 'chain_pitch': 0.08,
        'num_sprockets_upper': 12, 'num_sprockets_lower': 12,
        'num_blades': 24, 'blade_width': 0.3, 'blade_height': 0.15,
        'blade_thickness': 0.02, 'groove_depth': 0.12,
        'channel_width': 0.35, 'nominal_water_level_ratio': 0.6,
    }
    mat_dict = {
        'wood_density': 700.0, 'bamboo_density': 600.0,
        'iron_density': 7800.0, 'wood_elastic_modulus': 12e9,
        'iron_elastic_modulus': 200e9, 'wood_tensile_strength': 80e6,
        'iron_tensile_strength': 250e6, 'wood_friction_coeff': 0.4,
        'iron_friction_coeff': 0.15, 'water_density': 1000.0, 'gravity': 9.81,
    }
    sim_input_dict = {
        'rotational_speed': 15.0, 'water_level_diff': 2.0,
        'water_lift': 120.0, 'chain_wear_factor': 0.05,
    }

    with MechanicsWorker() as w:
        task_id = w.submit_task(sim_input_dict, geom_dict, mat_dict)
        assert task_id is not None
        assert len(task_id) > 0
        result = w.wait_result(task_id, timeout=5.0)
        assert result is not None
        assert result['status'] == 'success'
        assert 'result' in result
        assert result['elapsed_s'] >= 0
        assert 'overall_efficiency' in result['result']
        assert result['result']['overall_efficiency'] >= 0
        assert result['result']['drive_torque'] > 0
        assert result['result']['input_power'] > 0
...
def t6_4():
    ...
    main_pid = os.getpid()
    with MechanicsWorker() as w:
        task_id = w.submit_task(
            {'rotational_speed': 15.0, 'water_level_diff': 2.0, 'water_lift': 120.0, 'chain_wear_factor': 0.05},
            {'upper_wheel_diameter': 1.2, 'lower_wheel_diameter': 1.2, 'center_distance': 4.0,
             'chain_pitch': 0.08, 'num_sprockets_upper': 12, 'num_sprockets_lower': 12,
             'num_blades': 24, 'blade_width': 0.3, 'blade_height': 0.15,
             'blade_thickness': 0.02, 'groove_depth': 0.12,
             'channel_width': 0.35, 'nominal_water_level_ratio': 0.6},
            {'wood_density': 700.0, 'bamboo_density': 600.0, 'iron_density': 7800.0,
             'wood_elastic_modulus': 12e9, 'iron_elastic_modulus': 200e9,
             'wood_tensile_strength': 80e6, 'iron_tensile_strength': 250e6,
             'wood_friction_coeff': 0.4, 'iron_friction_coeff': 0.15,
             'water_density': 1000.0, 'gravity': 9.81},
        )
        result = w.wait_result(task_id, timeout=5.0)
        assert result['worker_pid'] != main_pid
        assert result['worker_pid'] > 0
        assert result['worker_pid'] == w._process.pid

def t6_5():
    from mechanics_worker import MechanicsWorker
    with MechanicsWorker() as w:
        tid1 = w.submit_task(
            {'rotational_speed': 10.0, 'water_level_diff': 1.0, 'water_lift': 100.0, 'chain_wear_factor': 0.05},
            {'upper_wheel_diameter': 1.2, 'lower_wheel_diameter': 1.2, 'center_distance': 4.0,
             'chain_pitch': 0.08, 'num_sprockets_upper': 12, 'num_sprockets_lower': 12,
             'num_blades': 24, 'blade_width': 0.3, 'blade_height': 0.15,
             'blade_thickness': 0.02, 'groove_depth': 0.12,
        ...
        tid2 = w.submit_task(
            {'rotational_speed': 30.0, 'water_level_diff': 5.0, 'water_lift': 200.0, 'chain_wear_factor': 0.05},
            {'upper_wheel_diameter': 1.2, 'lower_wheel_diameter': 1.2, 'center_distance': 4.0,
             'chain_pitch': 0.08, 'num_sprockets_upper': 12, 'num_sprockets_lower': 12,
             'num_blades': 24, 'blade_width': 0.3, 'blade_height': 0.15,
             'blade_thickness': 0.02, 'groove_depth': 0.12,
        ...
        r1 = w.wait_result(tid1, timeout=5.0)
        r2 = w.wait_result(tid2, timeout=5.0)
        assert r1 is not None
        assert r2 is not None
        assert r1['status'] == 'success'
        assert r2['status'] == 'success'
        assert r2['result']['input_power'] > r1['result']['input_power']
        assert r2['result']['drive_torque'] > r1['result']['drive_torque']
...
def t6_7():
    from mechanics_worker import MechanicsWorkerPool
    with MechanicsWorkerPool(num_workers=2) as pool:
        assert pool.num_workers == 2
        assert pool._workers[0].is_alive()
        assert pool._workers[1].is_alive()
...
def t6_8():
    ...
    sim = WaterWheelSimulator(geometry=geom, material=mat)
    local_result = sim.simulate(SimulationInput(**sim_input_dict))

    with MechanicsWorker() as w:
        task_id = w.submit_task(sim_input_dict, geom_dict, mat_dict)
        worker_result = w.wait_result(task_id, timeout=5.0)
        assert abs(worker_result['result']['overall_efficiency'] - local_result.overall_efficiency) < 0.001
...
def run_all_tests():
    ...
    print("\n[5] Refactored Modules - Unit Tests")
    test("[evolution_analyzer] module import & archaeology data", t5_1)
    test("[evolution_analyzer] dataclasses importable", t5_2)
    test("[era_comparator] module import & pump validation", t5_3)
    test("[era_comparator] operating range check", t5_4)
    test("[fleet_scheduler] module import & comm sim init", t5_5)
    test("[fleet_scheduler] comm dataclasses", t5_6)
    test("[fleet_scheduler] distance calculation km", t5_7)
    test("[vr_waterwheel] module import & force feedback", t5_8)
    test("[vr_waterwheel] session & leaderboard", t5_9)

    # [6] Mechanics Worker
    print("\n[6] Mechanics Worker - Unit Tests")
    test("[mechanics_worker] create worker not running", t6_1)
    test("[mechanics_worker] context manager start/stop", t6_2)
    test("[mechanics_worker] submit task & get result", t6_3)
    test("[mechanics_worker] runs in separate process", t6_4)
    test("[mechanics_worker] multiple tasks produce different results", t6_5)
    test("[mechanics_worker] submit before start raises error", t6_6)
    test("[mechanics_worker] worker pool init", t6_7)
    test("[mechanics_worker] result matches local simulation", t6_8)
    ...
    print(f"  Refactored modules:  9 tests")
    print(f"  Mechanics worker:    8 tests")
