...
export class EvolutionPanel {
    ...
    async loadEvolutionCompare() {
        try {
            const resp = await fetch(`${this.apiUrl}/api/evolution/compare`);
            this.evolutionData = await resp.json();
            return this.evolutionData;
        } catch (e) {
            console.error('加载演变对比数据失败:', e);
            return null;
        }
    }
    ...
    async loadTimeline() {
        try {
            const resp = await fetch(`${this.apiUrl}/api/evolution/timeline`);
            this.timelineData = await resp.json();
            return this.timelineData;
        } catch (e) {
            console.error('加载演变时间线失败:', e);
            return null;
        }
    }
    ...
    async loadScores() {
        try {
            const resp = await fetch(`${this.apiUrl}/api/evolution/scores`);
            this.scoresData = await resp.json();
            return this.scoresData;
        } catch (e) {
            console.error('加载演变评分失败:', e);
            return null;
        }
    }

    async loadSources(dynasty) {
        try {
            const resp = await fetch(`${this.apiUrl}/api/evolution/sources?dynasty=${dynasty}`);
            this.sourceData = await resp.json();
            return this.sourceData;
        } catch (e) {
            console.error('加载考古数据源失败:', e);
            return null;
        }
    }
    ...
    async simulateDynasty(dynasty, speed, waterLevel) {
        try {
            const resp = await fetch(`${this.apiUrl}/api/evolution/simulate`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ dynasty, speed, water_level: waterLevel })
            });
    ...
    renderComparison(container) {
        ...
        dynasties.forEach(key => {
            ...
            titleWrapper.style.cssText = 'display:flex;align-items:center;gap:10px;';
            ...
            confBadge.style.cssText = `
                ...
                background:${confidence > 0.8 ? 'rgba(16,185,129,0.2)' : confidence > 0.6 ? 'rgba(245,158,11,0.2)' : 'rgba(239,68,68,0.2)'};
                color:${confidence > 0.8 ? '#10b981' : confidence > 0.6 ? '#f59e0b' : '#ef4444'};
                font-weight:700;
            `;
            confBadge.textContent = `置信度 ${(confidence * 100).toFixed(0)}%`;
    ...
    renderSources(container, sources) {
        if (!sources || sources.length === 0) {
            container.innerHTML = '<div style="text-align:center;color:var(--text-muted);padding:20px;font-size:13px;">暂无考古数据源</div>';
            return;
        }
        ...
        container.innerHTML = sources.map((s, i) => {
            ...
            return `
                <div style="background:rgba(255,255,255,0.03);border:1px solid var(--border-color);border-radius:var(--radius-sm);padding:12px;margin-bottom:8px;">
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
                        <span style="font-size:16px;">${sourceTypeIcons[s.source_type] || '📄'}</span>
                        <span style="font-size:13px;font-weight:600;color:var(--text-primary);flex:1;">${s.reference}</span>
                        <span style="font-size:10px;color:var(--text-muted);">${s.year}年</span>
                    </div>
                    ...
                    <div style="display:flex;align-items:center;gap:8px;">
                        ...
                        <span style="font-size:10px;color:${confColor};font-weight:700;">${confPct.toFixed(0)}%</span>
                    </div>
                    ${s.notes ? `<div style="font-size:10px;color:var(--text-muted);margin-top:6px;">${s.notes}</div>` : ''}
                </div>
            `;
        }).join('');
    }
            ...
            options: {
                ...
                scales: {
                    r: {
                        beginAtZero: true,
                        max: 1,
                        ticks: { stepSize: 0.2, color: '#5c7080', backdropColor: 'transparent', font: { size: 10 } },
                        grid: { color: 'rgba(45, 66, 89, 0.4)' },
                        angleLines: { color: 'rgba(45, 66, 89, 0.4)' },
                        pointLabels: { color: '#8899a8', font: { size: 12, weight: '600' } }
                    }
                }
            }
        });
            ...
            options: {
                ...
                plugins: {
                    legend: {
                        position: 'top',
                        align: 'end',
                        labels: { color: '#8899a8', boxWidth: 12, padding: 15, font: { size: 12 } }
                    }
                },
                scales: {
                    x: { grid: { color: 'rgba(45, 66, 89, 0.3)' }, ticks: { color: '#8899a8', font: { size: 12 } } },
                    y: { grid: { color: 'rgba(45, 66, 89, 0.3)' }, ticks: { color: '#8899a8', font: { size: 10 } }, beginAtZero: true }
                }
            }
        });
    }
