...
export class EraComparatorPanel {
    ...
    async loadEfficiencyComparison(waterLevel, flowRate) {
        return this._fetch('/api/era/comparison/efficiency', {
            method: 'POST',
            body: JSON.stringify({ water_level: waterLevel, flow_rate_m3h: flowRate })
        });
    }
    ...
    async loadCostComparison(hours, flowRate, waterLevel) {
        return this._fetch('/api/era/comparison/costs', {
            method: 'POST',
            body: JSON.stringify({
                annual_operating_hours: hours,
                flow_rate_m3h: flowRate,
    ...
    async loadEnvironmentalImpact() {
        return this._fetch('/api/era/comparison/environmental');
    }

    async loadSummary() {
        return this._fetch('/api/era/comparison/summary');
    }

    async loadEfficiencyCurves(minSpeed, maxSpeed) {
        return this._fetch(`/api/era/comparison/curves?min_speed=${minSpeed}&max_speed=${maxSpeed}`);
    }

    async loadFullComparison(waterLevel, flowRate, hoursPerYear) {
        return this._fetch('/api/era/comparison/full', {
            method: 'POST',
            body: JSON.stringify({
                water_level: waterLevel,
                flow_rate_m3h: flowRate,
    ...
    async loadOperatingRange() {
        return this._fetch('/api/era/comparison/operating-range');
    }

    async validateOperatingPoint(flow, head) {
        return this._fetch('/api/era/comparison/validate-point', {
            method: 'POST',
            body: JSON.stringify({ flow_m3h: flow, head_m: head })
        });
    }

    renderOperatingPointValidation(container, validation) {
        const el = document.getElementById(container) || document.querySelector(container);
        if (!el) return;

        const isInRange = validation?.is_within_range;
        const warnings = validation?.warnings || [];
        const statusColor = isInRange ? '#10b981' : '#ef4444';
        const statusText = isInRange ? '✓ 工况点在运行域内' : '⚠ 工况点偏离设计范围';

        el.innerHTML = `
            <div style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-sm);padding:16px;">
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
                    <span style="font-size:18px;color:${statusColor};">${isInRange ? '✓' : '⚠'}</span>
                    <span style="font-size:14px;font-weight:600;color:${statusColor};">${statusText}</span>
                </div>
                ${warnings.length > 0 ? `
                    <div style="font-size:12px;color:var(--text-secondary);margin-bottom:8px;">告警信息:</div>
                    ${warnings.map(w => `
                        <div style="background:rgba(245,158,11,0.1);border-left:3px solid #f59e0b;padding:8px 12px;margin-bottom:4px;border-radius:4px;font-size:11px;color:var(--text-primary);">
                            ${w}
                        </div>
                    `).join('')}
                ` : ''}
                ${validation?.deviation_percent !== undefined ? `
                    <div style="margin-top:8px;font-size:11px;color:var(--text-muted);">
                        偏离度: <span style="color:${validation.deviation_percent > 20 ? '#ef4444' : validation.deviation_percent > 10 ? '#f59e0b' : '#10b981'};font-weight:700;">${validation.deviation_percent.toFixed(1)}%</span>
                    </div>
                ` : ''}
            </div>
        `;
    ...
    renderEfficiencyChart(canvasId, data) {
        ...
        const chartKey = `era_efficiency_${canvasId}`;
        if (this.charts[chartKey]) {
            this.charts[chartKey].destroy();
        }
    ...
    renderCostBreakdown(container, data) {
        ...
        const chartKey = `era_cost_${container}`;
        if (this.charts[chartKey]) {
            this.charts[chartKey].destroy();
        }

        const canvasId = `canvas-era-cost-${container.replace(/[^a-zA-Z0-9]/g, '')}`;
        el.innerHTML = `<canvas id="${canvasId}" style="width:100%;height:260px;"></canvas>`;
