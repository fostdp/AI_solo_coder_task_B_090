...
export class FleetSchedulerPanel extends SchedulingPanel {
    async addWheel(wheelData) {
        const result = await this._api('POST', '/api/fleet/wheels', wheelData);
        return result;
    }

    async addZone(zoneData) {
        const result = await this._api('POST', '/api/fleet/zones', zoneData);
        return result;
    }

    async loadWheelStatus() {
        const result = await this._api('GET', '/api/fleet/wheels');
        this.wheels = result.wheels || result || [];
        return this.wheels;
    }

    async loadZoneList() {
        const result = await this._api('GET', '/api/fleet/zones');
        this.zones = result.zones || result || [];
        return this.zones;
    }

    async optimizeSchedule(targetWater, hoursAvailable) {
        const result = await this._api('POST', '/api/fleet/optimize', {
            target_water: targetWater,
            hours_available: hoursAvailable
        });
        return result;
    ...
    async loadSchedule() {
        const result = await this._api('GET', '/api/fleet/schedule');
        this.schedule = result.schedule || result || null;
        return this.schedule;
    }

    async loadRecommendations() {
        const result = await this._api('GET', '/api/fleet/recommendations');
        this.recommendations = result.recommendations || result || [];
        return this.recommendations;
    }

    async resetScheduler() {
        const result = await this._api('POST', '/api/fleet/reset');
        this.wheels = [];
        this.zones = [];
        this.schedule = null;
        this.recommendations = [];
    ...
    async simulateDelay(wheelId, zoneId) {
        const result = await this._api('POST', '/api/fleet/simulate-delay', {
            wheel_id: wheelId,
            zone_id: zoneId
        });
        return result;
    ...
    async loadDelayConfig() {
        const result = await this._api('GET', '/api/fleet/delay-config');
        return result;
    }

    renderDelaySimulation(container, delayResult) {
        if (!container) return;
        if (!delayResult) {
            container.innerHTML = '<div style="text-align:center;color:var(--text-muted);padding:20px;font-size:13px;">暂无延迟模拟数据</div>';
            return;
        }

        const baseDelay = delayResult.base_delay || 0;
        const distanceDelay = delayResult.distance_delay || 0;
        const packetLoss = delayResult.packet_loss || 0;
        const totalDelay = delayResult.total_delay || 0;
        const retryCount = delayResult.retry_count || 0;

        container.innerHTML = `<div style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-sm);padding:16px;">
            <div style="font-size:14px;font-weight:600;color:var(--text-primary);margin-bottom:14px;">通信延迟模拟结果</div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                <div style="background:rgba(6,182,212,0.05);border:1px solid rgba(6,182,212,0.2);border-radius:8px;padding:12px;">
                    <div style="font-size:11px;color:var(--text-secondary);margin-bottom:4px;">基础延迟</div>
                    <div style="font-size:18px;font-weight:700;color:var(--accent-cyan);font-family:Consolas,monospace;">${baseDelay.toFixed(2)}<span style="font-size:10px;color:var(--text-muted);margin-left:4px;">ms</span></div>
                </div>
                <div style="background:rgba(59,130,246,0.05);border:1px solid rgba(59,130,246,0.2);border-radius:8px;padding:12px;">
                    <div style="font-size:11px;color:var(--text-secondary);margin-bottom:4px;">距离延迟</div>
                    <div style="font-size:18px;font-weight:700;color:var(--accent-blue);font-family:Consolas,monospace;">${distanceDelay.toFixed(2)}<span style="font-size:10px;color:var(--text-muted);margin-left:4px;">ms</span></div>
                </div>
                <div style="background:rgba(245,158,11,0.05);border:1px solid rgba(245,158,11,0.2);border-radius:8px;padding:12px;">
                    <div style="font-size:11px;color:var(--text-secondary);margin-bottom:4px;">丢包率</div>
                    <div style="font-size:18px;font-weight:700;color:var(--accent-yellow);font-family:Consolas,monospace;">${(packetLoss * 100).toFixed(1)}<span style="font-size:10px;color:var(--text-muted);margin-left:4px;">%</span></div>
                </div>
                <div style="background:rgba(16,185,129,0.05);border:1px solid rgba(16,185,129,0.2);border-radius:8px;padding:12px;">
                    <div style="font-size:11px;color:var(--text-secondary);margin-bottom:4px;">总延迟</div>
                    <div style="font-size:18px;font-weight:700;color:var(--accent-green);font-family:Consolas,monospace;">${totalDelay.toFixed(2)}<span style="font-size:10px;color:var(--text-muted);margin-left:4px;">ms</span></div>
                </div>
                <div style="grid-column:span 2;background:rgba(139,92,246,0.05);border:1px solid rgba(139,92,246,0.2);border-radius:8px;padding:12px;">
                    <div style="font-size:11px;color:var(--text-secondary);margin-bottom:4px;">重试次数</div>
                    <div style="font-size:18px;font-weight:700;color:var(--accent-purple);font-family:Consolas,monospace;">${retryCount}<span style="font-size:10px;color:var(--text-muted);margin-left:4px;">次</span></div>
                </div>
            </div>
        </div>`;
    }

    renderDelayConfig(container, config) {
        if (!container) return;
        if (!config) {
            container.innerHTML = '<div style="text-align:center;color:var(--text-muted);padding:20px;font-size:13px;">暂无延迟配置数据</div>';
            return;
        }

        container.innerHTML = `<div style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-sm);padding:16px;">
            <div style="font-size:14px;font-weight:600;color:var(--text-primary);margin-bottom:14px;">延迟配置参数</div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 10px;background:rgba(255,255,255,0.03);border-radius:6px;">
                    <span style="font-size:12px;color:var(--text-secondary);">基础延迟(ms)</span>
                    <span style="font-size:13px;font-weight:700;color:var(--accent-cyan);font-family:Consolas,monospace;">${config.base_delay || 0}</span>
                </div>
                <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 10px;background:rgba(255,255,255,0.03);border-radius:6px;">
                    <span style="font-size:12px;color:var(--text-secondary);">每米延迟(ms/m)</span>
                    <span style="font-size:13px;font-weight:700;color:var(--accent-blue);font-family:Consolas,monospace;">${config.delay_per_meter || 0}</span>
                </div>
                <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 10px;background:rgba(255,255,255,0.03);border-radius:6px;">
                    <span style="font-size:12px;color:var(--text-secondary);">基础丢包率</span>
                    <span style="font-size:13px;font-weight:700;color:var(--accent-yellow);font-family:Consolas,monospace;">${((config.base_packet_loss || 0) * 100).toFixed(1)}%</span>
                </div>
                <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 10px;background:rgba(255,255,255,0.03);border-radius:6px;">
                    <span style="font-size:12px;color:var(--text-secondary);">每公里丢包增量</span>
                    <span style="font-size:13px;font-weight:700;color:var(--accent-orange);font-family:Consolas,monospace;">${((config.packet_loss_per_km || 0) * 100).toFixed(2)}%</span>
                </div>
                <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 10px;background:rgba(255,255,255,0.03);border-radius:6px;">
                    <span style="font-size:12px;color:var(--text-secondary);">最大重试次数</span>
                    <span style="font-size:13px;font-weight:700;color:var(--accent-green);font-family:Consolas,monospace;">${config.max_retries || 0}</span>
                </div>
                <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 10px;background:rgba(255,255,255,0.03);border-radius:6px;">
                    <span style="font-size:12px;color:var(--text-secondary);">重试延迟系数</span>
                    <span style="font-size:13px;font-weight:700;color:var(--accent-purple);font-family:Consolas,monospace;">${config.retry_delay_factor || 0}</span>
                </div>
            </div>
        </div>`;
    }
